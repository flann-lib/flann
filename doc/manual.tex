...
\item[dists]{Matrix that will contain the distances to the K-nearest neighbors found. (size should be at least $num\_queries \times knn$)}
\item[knn]{Number of nearest neighbors to search for.}
\item[params]{Search parameters. Structure currently contains a single field, \texttt{checks} specifying the number 
of times the tree(s) in the index should be recursively traversed. A
