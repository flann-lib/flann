...
/+				if (i%100==0) {
	...
	query point q
	*/
	public void findNN(ResultSet result,float[] vec, int maxCheck, inout int checks, inout BranchHeap heap)
	{
		static int count = 0;
...
/+			if (count++<10) {
				writef("Pop branching cluster\n");
			}+/
			KMeansCluster cluster = branch.node;			
			cluster.findNN(result,vec, maxCheck,checks, heap);
		}
	}
