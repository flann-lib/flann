...
int main(string[] args)
{
	...
	if (args.length!=4) {
		writefln("Usage: ",args[0]," input_file test_file match_file");
	} else {

		writef("Reading input data... ");
		fflush(stdout);
		Features inputData = new Features();
		inputData.readFromFile(args[1]);
		writefln("done");
		
		writef("Reading test data... ");
		fflush(stdout);
		Features testData = new Features();
		testData.readFromFile(args[2]);
		writefln("done");

		writef("Computing ground truth... ");
		fflush(stdout);
		int[] matches = computeGroundTruth(inputData, testData);
		writefln("done");
	
	
		writef("Writing matches... ");
		fflush(stdout);
		writeMatches(args[3],matches);
		writefln("done");
