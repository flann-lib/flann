...
static PyObject* compute_ground_truth_cpp(PyObject*self, PyObject* args, PyObject* kywds)
{
    py::object return_val;
    int exception_occured = 0;
    PyObject *py_local_dict = NULL;
    static char *kwlist[] = {"dataset","testset","match","local_dict", NULL};
    PyObject *py_dataset, *py_testset, *py_match;
    int dataset_used, testset_used, match_used;
    py_dataset = py_testset = py_match = NULL;
    dataset_used= testset_used= match_used = 0;
    
    if(!PyArg_ParseTupleAndKeywords(args,kywds,"OOO|O:compute_ground_truth_cpp",kwlist,&py_dataset, &py_testset, &py_match, &py_local_dict))
       return NULL;
    try                              
    {                                
...
{
    {"compute_ground_truth_cpp",(PyCFunction)compute_ground_truth_cpp , METH_VARARGS|METH_KEYWORDS},
    {NULL,      NULL}        /* Sentinel */
};

PyMODINIT_FUNC initutil_c_c(void)
{
    
    Py_Initialize();
    import_array();
    PyImport_ImportModule("numpy");
    (void) Py_InitModule("util_c_c", compiled_methods);
}
