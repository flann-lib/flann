...
static PyObject* make_index(PyObject*self, PyObject* args, PyObject* kywds)
{
    py::object return_val;
    int exception_occured = 0;
    PyObject *py_local_dict = NULL;
    static char *kwlist[] = {"dataset","npts","dim","log_level","random_seed","algorithm","checks","trees","branching","iterations","target_precision","centers_init","local_dict", NULL};
    PyObject *py_dataset, *py_npts, *py_dim, *py_log_level, *py_random_seed, *py_algorithm, *py_checks, *py_trees, *py_branching, *py_iterations, *py_target_precision, *py_centers_init;
    int dataset_used, npts_used, dim_used, log_level_used, random_seed_used, algorithm_used, checks_used, trees_used, branching_used, iterations_used, target_precision_used, centers_init_used;
    py_dataset = py_npts = py_dim = py_log_level = py_random_seed = py_algorithm = py_checks = py_trees = py_branching = py_iterations = py_target_precision = py_centers_init = NULL;
    dataset_used= npts_used= dim_used= log_level_used= random_seed_used= algorithm_used= checks_used= trees_used= branching_used= iterations_used= target_precision_used= centers_init_used = 0;
    
    if(!PyArg_ParseTupleAndKeywords(args,kywds,"OOOOOOOOOOOO|O:make_index",kwlist,&py_dataset, &py_npts, &py_dim, &py_log_level, &py_random_seed, &py_algorithm, &py_checks, &py_trees, &py_branching, &py_iterations, &py_target_precision, &py_centers_init, &py_local_dict))
       return NULL;
    try                              
    {                                
        ...
        flannparams.log_level = log_level; // printf("log_level = %f\n", float(log_level));
        flannparams.random_seed = random_seed; // printf("random_seed = %f\n", float(random_seed));
        flannparams.log_destination = NULL;IndexParameters idxparams;
        idxparams.algorithm = algorithm; // printf("algorithm = %f\n", float(algorithm));
        idxparams.checks = checks; // printf("checks = %f\n", float(checks));
        idxparams.trees = trees; // printf("trees = %f\n", float(trees));
        idxparams.branching = branching; // printf("branching = %f\n", float(branching));
        idxparams.iterations = iterations; // printf("iterations = %f\n", float(iterations));
        idxparams.target_precision = target_precision; // printf("target_precision = %f\n", float(target_precision));
        idxparams.centers_init = centers_init; // printf("centers_init = %f\n", float(centers_init));
        
                    // printf("npts = %d, dim = %d", npts, dim);
                    float speedup = 1;
...
static PyObject* del_index(PyObject*self, PyObject* args, PyObject* kywds)
{
    py::object return_val;
    int exception_occured = 0;
    PyObject *py_local_dict = NULL;
    static char *kwlist[] = {"i","log_level","random_seed","local_dict", NULL};
    PyObject *py_i, *py_log_level, *py_random_seed;
    int i_used, log_level_used, random_seed_used;
    py_i = py_log_level = py_random_seed = NULL;
    i_used= log_level_used= random_seed_used = 0;
    
    if(!PyArg_ParseTupleAndKeywords(args,kywds,"OOO|O:del_index",kwlist,&py_i, &py_log_level, &py_random_seed, &py_local_dict))
       return NULL;
    try                              
    {                                
        py_i = py_i;
        int i = convert_to_int(py_i,"i");
        i_used = 1;
        py_log_level = py_log_level;
        int log_level = convert_to_int(py_log_level,"log_level");
        log_level_used = 1;
        ...
        flannparams.log_destination = NULL;
                    flann_free_index(FLANN_INDEX(i), &flannparams);
        if(py_local_dict)                                  
        {                                                  
            py::dict local_dict = py::dict(py_local_dict); 
        }                                                  
...
static PyObject* find_nn_index(PyObject*self, PyObject* args, PyObject* kywds)
{
    py::object return_val;
    int exception_occured = 0;
    PyObject *py_local_dict = NULL;
    static char *kwlist[] = {"nn_index","testset","tcount","result","num_neighbors","checks","log_level","random_seed","local_dict", NULL};
    PyObject *py_nn_index, *py_testset, *py_tcount, *py_result, *py_num_neighbors, *py_checks, *py_log_level, *py_random_seed;
    int nn_index_used, testset_used, tcount_used, result_used, num_neighbors_used, checks_used, log_level_used, random_seed_used;
    py_nn_index = py_testset = py_tcount = py_result = py_num_neighbors = py_checks = py_log_level = py_random_seed = NULL;
    nn_index_used= testset_used= tcount_used= result_used= num_neighbors_used= checks_used= log_level_used= random_seed_used = 0;
    
    if(!PyArg_ParseTupleAndKeywords(args,kywds,"OOOOOOOO|O:find_nn_index",kwlist,&py_nn_index, &py_testset, &py_tcount, &py_result, &py_num_neighbors, &py_checks, &py_log_level, &py_random_seed, &py_local_dict))
       return NULL;
    try                              
    {                                
        py_nn_index = py_nn_index;
        int nn_index = convert_to_int(py_nn_index,"nn_index");
        nn_index_used = 1;
        ...
        flannparams.log_level = log_level; // printf("log_level = %f\n", float(log_level));
        flannparams.random_seed = random_seed; // printf("random_seed = %f\n", float(random_seed));
        flannparams.log_destination = NULL;flann_find_nearest_neighbors_index(FLANN_INDEX(nn_index), testset, tcount,
...
static PyObject* find_nn(PyObject*self, PyObject* args, PyObject* kywds)
{
    py::object return_val;
    int exception_occured = 0;
    PyObject *py_local_dict = NULL;
    static char *kwlist[] = {"dataset","npts","dim","testset","tcount","result","num_neighbors","log_level","random_seed","algorithm","checks","trees","branching","iterations","target_precision","centers_init","local_dict", NULL};
    PyObject *py_dataset, *py_npts, *py_dim, *py_testset, *py_tcount, *py_result, *py_num_neighbors, *py_log_level, *py_random_seed, *py_algorithm, *py_checks, *py_trees, *py_branching, *py_iterations, *py_target_precision, *py_centers_init;
    int dataset_used, npts_used, dim_used, testset_used, tcount_used, result_used, num_neighbors_used, log_level_used, random_seed_used, algorithm_used, checks_used, trees_used, branching_used, iterations_used, target_precision_used, centers_init_used;
    py_dataset = py_npts = py_dim = py_testset = py_tcount = py_result = py_num_neighbors = py_log_level = py_random_seed = py_algorithm = py_checks = py_trees = py_branching = py_iterations = py_target_precision = py_centers_init = NULL;
    dataset_used= npts_used= dim_used= testset_used= tcount_used= result_used= num_neighbors_used= log_level_used= random_seed_used= algorithm_used= checks_used= trees_used= branching_used= iterations_used= target_precision_used= centers_init_used = 0;
    
    if(!PyArg_ParseTupleAndKeywords(args,kywds,"OOOOOOOOOOOOOOOO|O:find_nn",kwlist,&py_dataset, &py_npts, &py_dim, &py_testset, &py_tcount, &py_result, &py_num_neighbors, &py_log_level, &py_random_seed, &py_algorithm, &py_checks, &py_trees, &py_branching, &py_iterations, &py_target_precision, &py_centers_init, &py_local_dict))
       return NULL;
    try                              
    {                                
        py_dataset = py_dataset;
        ...
        idxparams.centers_init = centers_init; // printf("centers_init = %f\n", float(centers_init));
                    ...
                    // printf("npts = %d, dim = %d, tcount = %d", npts, dim, tcount);
...
static PyObject* run_kmeans(PyObject*self, PyObject* args, PyObject* kywds)
{
    ...
    int exception_occured = 0;
    PyObject *py_local_dict = NULL;
    static char *kwlist[] = {"dataset","npts","dim","num_clusters","result","log_level","random_seed","algorithm","checks","trees","branching","iterations","target_precision","centers_init","local_dict", NULL};
    PyObject *py_dataset, *py_npts, *py_dim, *py_num_clusters, *py_result, *py_log_level, *py_random_seed, *py_algorithm, *py_checks, *py_trees, *py_branching, *py_iterations, *py_target_precision, *py_centers_init;
    int dataset_used, npts_used, dim_used, num_clusters_used, result_used, log_level_used, random_seed_used, algorithm_used, checks_used, trees_used, branching_used, iterations_used, target_precision_used, centers_init_used;
    py_dataset = py_npts = py_dim = py_num_clusters = py_result = py_log_level = py_random_seed = py_algorithm = py_checks = py_trees = py_branching = py_iterations = py_target_precision = py_centers_init = NULL;
    dataset_used= npts_used= dim_used= num_clusters_used= result_used= log_level_used= random_seed_used= algorithm_used= checks_used= trees_used= branching_used= iterations_used= target_precision_used= centers_init_used = 0;
    
    if(!PyArg_ParseTupleAndKeywords(args,kywds,"OOOOOOOOOOOOOO|O:run_kmeans",kwlist,&py_dataset, &py_npts, &py_dim, &py_num_clusters, &py_result, &py_log_level, &py_random_seed, &py_algorithm, &py_checks, &py_trees, &py_branching, &py_iterations, &py_target_precision, &py_centers_init, &py_local_dict))
       return NULL;
    try                              
    {                                
        py_dataset = py_dataset;
        ...
        idxparams.centers_init = centers_init; // printf("centers_init = %f\n", float(centers_init));
                    ...
                    // printf("npts = %d, dim = %d, num_clusters = %d", npts, dim, num_clusters);
...
static PyMethodDef compiled_methods[] = 
{
    ...
    {"make_index",(PyCFunction)make_index , METH_VARARGS|METH_KEYWORDS},
    {"del_index",(PyCFunction)del_index , METH_VARARGS|METH_KEYWORDS},
    {"find_nn_index",(PyCFunction)find_nn_index , METH_VARARGS|METH_KEYWORDS},
    {"find_nn",(PyCFunction)find_nn , METH_VARARGS|METH_KEYWORDS},
    {"run_kmeans",(PyCFunction)run_kmeans , METH_VARARGS|METH_KEYWORDS},
    {NULL,      NULL}        /* Sentinel */
};

PyMODINIT_FUNC initflann_python_base(void)
{
    
    Py_Initialize();
    import_array();
    PyImport_ImportModule("numpy");
    (void) Py_InitModule("flann_python_base", compiled_methods);
}
