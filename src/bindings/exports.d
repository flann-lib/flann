...
NN_INDEX fann_build_index(float* dataset, int rows, int cols, int* speedup, IndexParameters* index_params, FANNParameters* fann_params)
{	
	try {
		fann_init();
		...
		if (target_precision < 0) {
			...
			*speedup = params["speedup"].get!(int);
		}
