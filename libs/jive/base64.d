...
/* Formatter that encodes input with base64 */

alias GenericFormatter!(encodeF_impl, char) encodeF;

package uint encodeF_impl (T) (T value, char[] format, Sink!(char) sink)
{
	char[3] buf;
	size_t saved = 0;

	uint sinkwrap (char[] s)
	{
		int drained = 0;
		size_t ret = 0;
		...
		if (saved > 0)
		{
			...
			{
				drained = 3 - saved;
				buf[saved .. 3] = s[0 .. 3 - saved];
				char[4] o;
				//Stdout.format ("<{}>", buf);
				ret += sink (encode (buf, o));
				saved = 0;
			}
			else
			{
				buf[saved .. saved+s.length] = s;
				saved += s.length;
				return 0;
			}
		}

		int len1 = (s.length - drained) - (s.length - drained) % 3;

		if (len1 > 0)
		{
			char[] o = (cast(char*) alloca (len1)) [0 .. len1];
			//Stdout.format ("<{}>", s[drained..drained+len1]);
			ret += sink (encode (s[drained .. drained+len1]));
		}

		int len2 = cast(int)s.length - len1 - drained;
		if (len2 > 0)
		{
			buf[saved .. saved+len2] = s[$-len2 .. $];
			saved += len2;
		}
	...
	size_t res = convert (&sinkwrap, format, value);

	if (saved > 0)
	{
		char[4] o;
		res += sink (encode (buf[0..saved], o));
	}

	return res;
}

/* Formatter that decodes input with base64 */

alias GenericFormatter!(decodeF_impl, char) decodeF;

package uint decodeF_impl (T) (T value, char[] format, Sink!(char) sink)
{
	char[4] buf;
	size_t saved = 0;
	...
	uint sinkwrap (char[] s)
	{
		...
		if (saved > 0)
		{
			...
			{
				drained = 4 - saved;
				buf[saved .. 4] = s[0 .. 4 - saved];
				char[3] o;
				//Stdout.format ("<{}>", buf);
				ret += sink (decode (buf, o));
				saved = 0;
			}
			else
			{
		...
		int len1 = (s.length - drained) - (s.length - drained) % 4;

		if (len1 > 0)
		{
			char[] o = (cast(char*) alloca (len1)) [0 .. len1];
			//Stdout.format ("<{}>", s[drained..drained+len1]);
			ret += sink (decode (s[drained..drained+len1]));
		}
	...
	{
		char[3] o;
		//Stdout.format ("<{}>", buf[0..saved]);
		res += sink (decode (buf[0..saved], o));
	}
