/******************************************************************************
    ...
    Based on phobos std.metastrings

    Original copyright notice at the end of the file

******************************************************************************/

module jive.metastrings;
 ...
 * Formats:
 *	The formats supported are $ for strings, and $$
 *	for the $ character.
 * Example:
 * ---
import jive.metastrings;
import jive.io;

void main()
{
  char[] s = Format!("Arg $ = $", "foo", 27);
  writeln(s); // "Arg foo = 27"
}
 * ---
 */
...
template FormatString(string F, A...)
{
	...
	{
		static assert (A.length == 0, "Too many parameters to Format!()");
		const char[] FormatString = Format!(A);
	}
	else static if (F[0] == '$' && (F.length == 1 || F[1] != '$'))
	{
		static assert (A.length > 0, "Too few parameters to Format!()");
		const char[] FormatString = ToString!(A[0]) ~ FormatString!(F[1..$],A[1..$]);
	}
	else static if (F.length > 1 && F[0..2] == "$$")
	{
